...
import com.act.monitor.model.TabAlarmSysConfig;
import com.act.web.module.common.service.AlarmSysConfigService;
 ...
 */
@Service
public class AlarmSysConfigServiceImpl implements AlarmSysConfigService {

	private final static Logger log = LoggerFactory
			.getLogger(AlarmSysConfigServiceImpl.class);
	...
	public List<SystemConfigVo> initialize() {
		List<SystemConfigVo> result = DbUtil
				.queryForObjectList(
						" select a.configid,a.defaultval,a.configval,a.title,a.showtype from tab_alarm_sys_config a where a.hidden = 1 order by a.showseq asc ",
	...
	public void resetConfig() {
		DbUtil.update(" update tab_alarm_sys_config  set configval = defaultval where hidden = 1 ");
	}
	...
	public void updateConfig(ConfigsVo sysList) throws Exception {
		...
		for (String sysConfig : sysConfigs) {
			if (sysConfig != null && sysConfig.trim().length() > 0) {
				TabAlarmSysConfig sys = JsonPluginsUtil.jsonToBean(sysConfig,
						TabAlarmSysConfig.class);
				DbUtil.update(
						" update tab_alarm_sys_config  set configval = ? where  configid = ? ",
	...
	public Map<String, String> loadAlarmConfig() {
		Map<String, String> result = new HashMap<String, String>();
		List<SystemConfigVo> sysList = DbUtil
				.queryForObjectList(
						" select a.configid,a.defaultval,a.configval,a.remark,a.showtype from tab_alarm_sys_config a ",
						SystemConfigVo.class);
