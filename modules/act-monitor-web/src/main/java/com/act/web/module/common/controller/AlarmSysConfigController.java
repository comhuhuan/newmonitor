...
@Controller
@RequestMapping("/common/alarmSysConfig")
public class AlarmSysConfigController extends BaseController {

    private final Logger log = LoggerFactory.getLogger(AlarmSysConfigController.class);

    /**
     * 对应SystemConfigService中 evictConfig方法 loadConfig缓存的key
     */
    private String LOAD_CONFIG = "loadEuConfig";

    @Resource
    private AlarmSysConfigService alarmSysConfigService;
    ...
    public Object initialize() {
        try {
            List<SystemConfigVo> result = alarmSysConfigService.initialize();
    ...
    public Object updateConfig(ConfigsVo sysList) {
        String result = "";
        try {
            alarmSysConfigService.updateConfig(sysList);
            alarmSysConfigService.evictConfig(LOAD_CONFIG);
            return ajax(Status.success, result);
        } catch (Exception e) {
    ...
    public Object resetConfig() {
        String result = "";
        try {
            alarmSysConfigService.resetConfig();
            alarmSysConfigService.evictConfig("loadEuConfig");
            return ajax(Status.success, result);
        } catch (Exception e) {
