...
@Service
public class CuSysConfigServiceImpl implements CuSysConfigService {

	private final static Logger log = LoggerFactory
			.getLogger(CuSysConfigServiceImpl.class);
	...
	public List<SystemConfigVo> initialize() {
		List<SystemConfigVo> result = DbUtil
				.queryForObjectList(
						" select a.configid,a.defaultval,a.configval,a.title,a.showtype from cu_sys_config a where a.hidden = 1 order by a.showseq asc ",
	...
	public void resetConfig() {
		DbUtil.update(" update cu_sys_config  set configval = defaultval where hidden = 1 ");
	}
	...
	public void updateConfig(ConfigsVo sysList) throws Exception {
		...
		for (String sysConfig : sysConfigs) {
			if (sysConfig != null && sysConfig.trim().length() > 0) {
				CuSysConfig sys = JsonPluginsUtil.jsonToBean(sysConfig,
						CuSysConfig.class);
				DbUtil.update(
						" update cu_sys_config  set configval = ? where  configid = ? ",
	...
	public Map<String, String> loadCuConfig() {

		Map<String, String> result = new HashMap<String, String>();
		List<SystemConfigVo> sysList = DbUtil
				.queryForObjectList(
						" select a.configid,a.defaultval,a.configval,a.remark,a.showtype from cu_sys_config a ",
						SystemConfigVo.class);
