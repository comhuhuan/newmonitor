            ...
            <el-col :span="24" class="main">
                        ...
                        <template v-for="(item,index) in $router.options.routes" v-if="!item.hidden">
                            <el-submenu :index="index+''">
                                ...
                                <el-menu-item v-for="child in item.children"
                                              ...
                                              v-if="!child.hidden"
                                              :key="index"
                                              :class="{'is-active' : child.name == activeName }"
                                >
                ...
                <section class="content-container">
                    <div class="grid-content bg-purple-light">
                        ...
                        <el-tabs :value="activeName" type="card" @tab-remove="handleRemoveTab"
                                 @tab-click="changeActiveName" closable>
                            <el-tab-pane v-for="(item, index) in this.activeMenu" :key="index" :label="item.title"
                                         :name="item.name">
                                <!-- :name="item.name"  -->
...
export default {
  ...
  methods: {
    ...
    logout: function () {
      this.$confirm('确认退出吗?', '提示', {
        ...
        storage.remove('storageTabMenus')
        location.replace('')
      }).catch(() => {
    ...
    handleRemoveTab (tab) {
      ...
      /* if (this.activeMenu.length > 0) {
                ...
                this.activeName = this.activeMenu[this.activeMenu.length - 1].title;
            } */
    },
    ...
    handleSelect: function (index, indexPath) {
      // index: 选中菜单项的path ; indexPath: 选中菜单项的 index 与 path
    ...
    handleCollSelect: function (index, indexPath) {
      ...
      const colltab = {title: colltitle, name: colltitle, component: component}
      this.addActive(colltab)
      // this.activeName = indexPath;
      this.activeName = colltitle
    },
    // name = 0 获取title  name = 1 获取component
  ...
  mounted () {
    ...
    let unitSystemVersion = sessionStorage.getItem('unitSystemVersion')
    if (unitSystemVersion && unitSystemVersion != null) {
      unitSystemVersion = JSON.parse(unitSystemVersion)
      this.sysVersion = unitSystemVersion.systemVersion || ''
    }
    let tabSysManageInfo = sessionStorage.getItem('tabSysManageInfo')
    if (tabSysManageInfo && tabSysManageInfo != null) {
      tabSysManageInfo = JSON.parse(tabSysManageInfo)
